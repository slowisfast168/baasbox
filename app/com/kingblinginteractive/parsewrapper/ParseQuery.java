...
public class ParseQuery
{
	private String mClassName; // the name of the Parse class to perform the
								// query on
	private SimpleEntry mWhereEqualTo = null; // tracks "WHERE" equal constraints
	private SimpleEntry mWhereLessThen = null; // tracks "WHERE" less constraints
	private String mWhereRawString = null; // tracks "WHERE" raw string constraints
	...
	{
		return get(theObjectId, false);
	}

	public ParseObject get(String theObjectId, boolean isUser) throws ParseException
	{
		String query = "";
		if(isUser)
		{
			query = Parse.getParseAPIUrlUsers()	+ "/"
					+ theObjectId;
		}
		else
		{
			query = Parse.getParseAPIUrlClasses() + mClassName + "/"
					+ theObjectId;
		}
	...
	public List<ParseObject> find(boolean isUser) throws ParseException
	{
		String query = "";
		if(isUser)
		{
	...
	/**
	 * Add a raw string to parse restful api
	 * the format is like below: '{"score":{"$gte":1000,"$lte":3000}}'
	 * @return Returns the query, so you can chain this call.
	 */
	public ParseQuery whereRawString(String rawQueryString)
	{
		mWhereRawString = rawQueryString;
		return this;
	}
	...
	{
		mWhereLessThen = new SimpleEntry<String, Object>(key, value);
		return this;
	}
	...
	{
		return mWhereEqualTo != null || mWhereLessThen !=null || mWhereRawString != null;
	}
	...
	private String getJSONWhereConstraints()
	{
		...
		if (mWhereRawString != null)
		{
			return mWhereRawString;
		}
		...
		if (mWhereLessThen != null)
		{
			...
			try
			{
				joLess.put("$lt", mWhereLessThen.getValue());
				jo.put(((String) mWhereLessThen.getKey()), joLess);
				js = jo.toString();
			}
			catch (JSONException e)
			{
